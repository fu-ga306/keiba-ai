...
実行: python perm_top2.py → perm_top2_result.csv
"""
import itertools
import warnings
...
N_PERM = 400
MIN_PT = 100
# 2026-08-16: 点数だけの足切りでは不十分だった。馬単は的中率2-5%なので
# 100点=的中2-5本のセルが最大値を取り、帰無分布が不当に高く出る。
# 的中数にも下限を設けて、実測・帰無の双方で「測れるセル」だけを比べる。
MIN_HIT = 30
...
def main():
    ...
    pd.DataFrame([{"実測最良ROI": round(obs, 1),
        ...
        .to_csv("perm_top2_result.csv", index=False, encoding="utf-8-sig")
