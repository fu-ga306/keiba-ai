...
# 1600万下=3勝 / 1000万下=2勝 / 500万下=1勝（2019年までの呼び方）


# 全角→半角。ページは「１０００万下」のように全角で書くことがある
_Z2H = str.maketrans("０１２３４５６７８９ＧＩＬ　",
                     "0123456789GIL ")
...
def _extract_class(text):
    ...
    ⚠ 全角に注意（2026-09-02）
      特別戦の条件は「３歳以上 １０００万下」と全角で書かれる。
      半角で探すと当たらず、10-12Rのクラスが取れていなかった。
      レース名側も「ライラック賞(2勝)」のように "クラス" が付かない。
    """
    if not text:
        return None
    ...
    for w in _CLASS_WORDS:
        if w in t:
            return w
    # 「(2勝)」「3歳以上2勝」のような、クラスが付かない書き方
    m = re.search(r"([1-3])\s*勝", t)
    if m:
        return m.group(1) + "勝クラス"
    return None
