...
def main():
    ...
    for src, note in (("history_marks.csv", "過去ぶん"),
        ...
        for r in t.itertuples():
            ch = pd.to_numeric(r.着順, errors="coerce")
            if pd.notna(ch):
                RANK[(r.race_id, r.馬番)] = float(ch)
    ...
    # 馬券内率(⑤)を出すために着順も引く。単勝の組み合わせ＝馬番なのでそのまま使える。
    b["着順"] = [RANK.get((r, c)) if k == "単勝" else None
                for r, k, c in zip(b.race_id, b["券種"], b["組み合わせ"])]
    ...
    # ── ⑤ 馬券内率（回収率より速く収束する指標）──────────────────────
    #   なぜ主指標を変えたか（2026-08-24）
    #     回収率で「100%超」の95%下限を出すには約6,400点＝約5年かかる。
    #     単勝1点の標準偏差は758で、平均117に対して大きすぎる。
    #     的中率14.4%・中央値0円で、平均は稀な大穴が支えている構造のため。
    #     同じ軸を複勝で買う案も試したが回収率86.6%で赤字（分散は1/3.5に
    #     下がるが控除率を越えられない）。低分散の代替手段が無い。
    #   そこで実測の役割を「黒字の証明」から「BTと矛盾していないかの監視」に
    #   変える。馬券内率なら100点（約1か月）で±9pt、200点で±6.5ptに収まる。
    #   壊れの検出にはこれで足りる。
    BT_PLACE = 32.1          # BTの買い目7,135点の馬券内率
    ax = b[b["券種"] == "単勝"].copy()
    if len(ax) and "着順" in ax.columns:
        pl = pd.to_numeric(ax["着順"], errors="coerce")
        n_pl = int(pl.notna().sum())
        if n_pl:
            r = (pl <= 3).sum() / n_pl * 100
            se = (BT_PLACE / 100 * (1 - BT_PLACE / 100) / n_pl) ** 0.5 * 100
            lo, hi = BT_PLACE - 1.96 * se, BT_PLACE + 1.96 * se
            log("\n=== ⑤ 馬券内率（回収率より速く収束する）===")
            log(f"  実測 {r:.1f}%（{n_pl}点）  BT {BT_PLACE}%")
            log(f"  BTが正しい場合に{n_pl}点で入る範囲 [{lo:.1f}%, {hi:.1f}%]")
            log(f"  → {'範囲内。矛盾なし' if lo <= r <= hi else '⚠ 範囲外。モデルの劣化を疑う'}")

    # ── 判断の目安 ──────────────────────────────────────
    hit = int((b.払戻 > 0).sum())
    npt = len(b[b["券種"] == "単勝"])
    log("\n=== 判断の目安 ===")
    log(f"  単勝{npt}点・的中{hit}本。")
    log("  ⚠ 回収率で黒字を証明するのは事実上できない。必要点数は次のとおり:")
    log("      1,600点(約14.5か月) 95%下限 88%")
    log("      3,200点(約29か月)   95%下限 95%")
    log("      6,400点(約58か月)   95%下限 101%  ← ようやく100%超")
    log("  なので実測は『黒字の証明』ではなく『BTと矛盾しないかの監視』に使う。")
    log("  黒字の根拠は BT（順列検定 p=0.0000）と odds_timing_bt.py（7分前でも")
    log("  -0.8pt）に置く。実測で見るのは上の④⑤と買い率のズレ。")
    log(f"\n  {ndays}開催日で{nrace:,}レース記録。")
