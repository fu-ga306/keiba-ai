...
有料にするのは「合言葉」だけ
  本文（先週の実績・較正表）は無料にする。売っているのは情報ではなく
  閲覧ページへの鍵であり、較正の実測表は購入の**根拠**そのものだから。
  中身が見えない300円は買えない。隠すと判断材料が無くなる。
...
def build(d, url, phrase, label):
    ...
    A("")
    A("ページはこちらです。**買う前に中身を見てください。**")
    A("")
    A(f"{url}")
    A("")
    A(f"{label} の各日、朝に更新されます。")
    A("合言葉なしでも上位3頭までは表示されます。")
    A("")
    ...
    A("")
    A("<!-- ===== ここから有料 ===== -->")
    A("<!-- note編集画面では、この直前の行で「ここから先は有料」に設定する -->")
    A("")
    A("## ご購入後のご案内")
    A("")
    ...
    A(f"**合言葉**： `{phrase}`")
    A("")
    A(f"この合言葉は **{label} の2日間**でお使いいただけます。")
    A("")
    A("※ レースごとにページが分かれています。各日の朝に更新されます。")
    A("※ 合言葉は開催ごとに変わります。前回のものもしばらく有効です。")
    A("")
    A("---")
    A("")
    A("**この表について**")
