...
#     実運用の成績を貯めて、半年後に判定する。
# ── 購入の大元スイッチ（2026-08-17）──────────────────────────────────
#   False にすると、どの方式が有効でも一切買わない。_race_bet_plan の先頭で
#   効くので、経路を1つ切り忘れても買わない。予想の表示・記録・オッズ収集は
#   これまでどおり続く。
#
#   いま False にしている理由:
#     検証の入力(bet_cache)が本番と別のモデルで作られていた。本番と同じ距離分割
#     で作り直したところ、採用していた買い方が全部100%を割った。
#       荒れR 軸1-10倍  5年 110.0% → 68.5%   直近2年 103.0% → 46.5%
#       妙味★ 単勝      5年  83.1%           直近2年  81.2%
#       事前登録9仮説    100%超は 0件（ROIは78〜89%に収束＝控除率どおり）
#     「100%を超えたものだけ採用」という基準に照らすと、買う対象が無い。
#
#   再開の条件:
#     ① harness.py で 5年・直近2年・7分前スリッページの3つとも100%超
#     ② check_impl.py が本番と検証の一致を報告する
#     ③ 使う bet_cache が本番と同じモデルで作られている（prep_cache.py の
#        距離分割・パラメータが train_mf_v2.py と揃っていること）
BETTING_ENABLED = False
BETTING_OFF_REASON = "購入停止中（検証で100%超の買い方が無いため・2026-08-17）"

# ── 2026-08-17: 荒れR方式を停止した ──────────────────────────────────
#   検証の入力(bet_cache)が本番と別のモデルで作られていたことが判明したため、
#   本番と同じ距離分割を入れて作り直したところ、成績が全部消えた。
#
#     軸1-10倍   5年 110.0% → 68.5%   直近2年 103.0% → 46.5%
#     10段階のオッズ上限すべてで100%割れ。100%超の構成は3件 → 0件。
#
#   採用の根拠だった「大穴を切ると良くなる」も消えた。切った側(10-20倍)の
#   ROIが82.2%で、残した側(1-10倍)の68.5%より高い。理屈が逆を向いた。
#
#   つまり110.0%はモデル不一致が作った幻で、実際に買っていれば負けていた。
#   検証が正しくなった結果、買う理由が無くなったので止める。
#   ⚠ 再開するなら check_impl.py が通ること、かつ harness.py で
#     5年・直近2年・7分前スリッページの3つとも100%を超えることが条件。
USE_ARE_UMATAN = False            # 荒れR方式（下の ARE_PLANS をまとめて実行）
...
# ── 2026-08-17: 妙味★方式も停止した ─────────────────────────────────
#   荒れR方式を止めたあと、本日の買い目が0にならないので調べたところ、
#   この経路が生きていた。購入経路の見落としはこれで2回目（前回は8/15）。
#
#   本番と同じ距離分割で作り直したキャッシュで測ると、
#     乖離>=3 × モデル3位以内 × 20倍以下 の単勝
#       5年 83.1%  /  直近2年 81.2%（年別 77.4 / 76.5 / 98.8 / 82.1 / 80.4）
#   上の「97.3%」は3年・別条件での値で、いま再現すると100%には遠い。
#   ※この再現は印(◎○▲)の絞りを含まないので厳密には本番と同じではないが、
#     20ポイント近い差はその違いでは説明できない。
#
#   「100%を超えたものだけ採用」という基準に照らして止める。
USE_MYOMI_BETTING = False         # False で従来のゲート方式に戻す
...
def _race_bet_plan(pdf):
    ...
    # ── 購入の大元スイッチ（2026-08-17追加）─────────────────────────
    #   これまで購入経路が4つあり、個別フラグを切っても別の経路が買っていた。
    #   USE_MYOMI_BETTING=False は「購入停止」ではなく「従来のゲート方式に戻す」
    #   なので、切ったつもりで12レース買っていた。見落としは2回目（前回8/15）。
    #   どの経路より先に効く1つのスイッチをここに置く。
    if not BETTING_ENABLED:
        plan["理由"] = BETTING_OFF_REASON
        return plan
