...
KEEP = [
    ...
    # 残差モデルの gap（2026-08-24追加）。全馬ぶん残す。
    #   なぜ要るか: 回収率で「黒字」を示すには約6,400点＝5年かかる（分散が
    #   大きすぎる。単勝1点の標準偏差758に対し平均117）。実測での黒字証明は
    #   現実的でないので、代わりに ΔR²（市場に対する情報量の上乗せ）を見る。
    #   ΔR² は買った26点ではなく全レース×全馬を使うので、検出力が桁違いに高い。
    #   ただし計算には全馬の残差確率が要る。paper_resid.csv は軸と相手しか
    #   持っていないので、ここで全馬ぶんを残しておく必要がある。
    "resid_gap",
]
