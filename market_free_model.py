...
FEATURE_COLS_MF = [
    ...
    # 精密展開特徴（2026-07-27）は検証で悪化したためMFでも未使用。
    #   同条件BT: 全体198.5%→191.8%。妙(勝率)の複勝率は50.0→52.0%と改善したが、
    #   複妙(複勝)が57.9→57.7%と劣化し相手プールが弱まり、馬単で大きく損失。
    # 開催時期
    "開催月", "開催季節",
    # 前走情報
    ...
    # 血統適性6特徴は自己リーク(学習行自身の結果がsire集計に混入)で毒と判明し除去。
    #   2025: 軸複勝率52.85→58.54% / 2024(集計<=2023): 52.61→61.69%, 勝率AUC0.667→0.788
    #   (2026-07-28検証・explore_tiered.py/confirm_blood_2024)。再導入するなら
    #   レース日時点までの expanding 集計に作り直すこと。
]
